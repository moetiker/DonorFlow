...
const STRINGS: Record<EmailLocale, Record<string, string>> = {
  de: {
    ...
    intro: 'Im Anhang finden Sie unseren Gönnerbrief für das Vereinsjahr {year}. Hier Ihr aktueller Spendenstand:',
    target: 'Ziel',
    collected: 'Gesammelt',
    remaining: 'Offen',
    ofTarget: '{pct}% des Ziels erreicht',
    button: 'Meinen Status ansehen',
    attachments: 'Angehängt: Gönnerbrief (PDF) und Ihre Adressliste (CSV).',
  ...
  fr: {
    ...
    attachments: 'En pièce jointe : la lettre aux donateurs (PDF) et votre liste d’adresses (CSV).',
    footer: 'Cet e-mail a été envoyé automatiquement par {org}.',
  },
  it: {
    subject: 'Lettera ai donatori {org} – {year}',
    greeting: 'Buongiorno {name}',
    intro: 'In allegato trova la nostra lettera ai donatori per la stagione {year}. Ecco lo stato attuale delle donazioni:',
    target: 'Obiettivo',
    collected: 'Raccolto',
    remaining: 'Aperto',
    ofTarget: '{pct}% dell’obiettivo raggiunto',
    button: 'Vedi il mio stato',
    attachments: 'In allegato: la lettera ai donatori (PDF) e il suo elenco indirizzi (CSV).',
...
export function renderStatusEmail(data: StatusEmailData): {
  ...
  const vars = { org: data.orgName, year: data.fiscalYearName, name: data.memberName, pct: data.progress.percentage }
  ...
  <tr>
    <td align="center" style="padding:24px 12px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:100%;background:#ffffff;border-radius:12px;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
        <!-- header -->
        <tr>
          <td style="background:#1a3f7a;padding:20px 28px;color:#ffffff;font-size:18px;font-weight:700;">
        ...
        <tr>
          <td style="padding:28px;color:#1f2a37;">
            ...
            <p style="margin:0 0 22px;font-size:15px;line-height:1.55;color:#414d5b;">${escapeHtml(fill(s.intro, vars))}</p>
              ...
              <tr>
                <td style="background:#e6ebf2;border-radius:999px;padding:0;">
                  <table role="presentation" width="${pct}%" cellpadding="0" cellspacing="0" style="min-width:8px;">
                    <tr><td style="background:#0d6efd;border-radius:999px;height:12px;line-height:12px;font-size:0;">&nbsp;</td></tr>
                  </table>
                </td>
              </tr>
            </table>
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 8px;">
              <tr>
                <td align="center" style="background:#0d6efd;border-radius:8px;">
            ...
            ${data.attachmentsNote ? `<p style="margin:20px 0 0;font-size:13px;line-height:1.5;color:#77828f;">📎 ${escapeHtml(s.attachments)}</p>` : ''}
          </td>
        </tr>
        <!-- footer -->
        <tr>
